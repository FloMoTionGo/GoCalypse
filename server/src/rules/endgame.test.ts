 ...
 * player's grey pattern stone, 9 driftwood. Player 1 is black+dots, 2 white+dots,
 * 3 black+stripes, 4 white+stripes.
 */
...
test("an empty board scores nothing for anyone, and everyone shares first place", () => {
  const a = area(["...", "...", "..."]);
  assert.deepEqual(a, { black: 0, white: 0, dots: 0, stripes: 0 });
...
test("stones count, empty points count for the one side that walls them in, shared points count for no one", () => {
  ...
  assert.equal(a.stripes, 0);
});

test("a stone on the other front is a wall: it scores nothing here, and does not spoil a region", () => {
  // The grey dots stone counts on the pattern front only.
  const onlyPattern = area(["5..", "...", "..."]);
  assert.deepEqual(onlyPattern, { black: 0, white: 0, dots: 9, stripes: 0 });

  // A solid black stone beside a grey dots stone: the black front sees a black
  // stone and one wall, so the whole empty region is black's; the pattern front
  // sees the mirror image and hands the same region to dots.
  const both = area(["15.", "...", "..."]);
  assert.deepEqual(both, { black: 1 + 7, white: 0, dots: 1 + 7, stripes: 0 });
});

test("driftwood is a wall on both fronts", () => {
  const a = area(["19.", "...", "..."]);
  assert.deepEqual(a, { black: 8, white: 0, dots: 0, stripes: 0 });
});

test("the two fronts are counted independently on the same points", () => {
  // Columns: black solid stones, grey stripes stones, white solid stones, then
  // an empty column.
  const a = area([
    "172.",
    "172.",
  ...
  // only white, so it is white's too. The stripes stones are walls here.
  assert.equal(a.black, 4);
  ...
  assert.equal(a.stripes, 4);
  assert.equal(a.dots, 0);
});

test("everyone belongs to one side on each front", () => {
  assert.deepEqual(sidesOf(1), { base: "black", pattern: "dots" });
  assert.deepEqual(sidesOf(2), { base: "white", pattern: "dots" });
  assert.deepEqual(sidesOf(3), { base: "black", pattern: "stripes" });
  assert.deepEqual(sidesOf(4), { base: "white", pattern: "stripes" });
});

test("a player's final score is the lower of their two sides, and the higher one breaks ties", () => {
  const results = finalResults({ black: 10, white: 6, dots: 8, stripes: 12 }, [1, 2, 3, 4]);
...
test("players level on score and tiebreak share a place, and the next place is skipped", () => {
  const results = finalResults({ black: 9, white: 3, dots: 9, stripes: 3 }, [1, 2, 3, 4]);
...
test("results come back in the order the combos were given", () => {
  const results = finalResults({ black: 1, white: 2, dots: 3, stripes: 4 }, [4, 1]);
